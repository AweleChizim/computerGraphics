...
void updateBall() {
	...
	if (ball.a < 0 || ball.a + BALL_RADIUS > SCREEN_WIDTH) {
		ball.da *= -1;
		printf("a Value is: %d\n", ball.b);
	}
	if (ball.b < 0 || ball.b + BALL_RADIUS > SCREEN_HEIGHT) {
		ball.db *= -1;
		printf("b Value is: %d\n", ball.b);
...
int main(int argc, char* argv[]) {
	...
	*/
	...
	while (!quit) {
		...
		SDL_Rect ballRect2 = { ball.a, ball.b, BALL_RADIUS, BALL_RADIUS };
		SDL_RenderCopy(gRenderer, ballText, NULL, &ballRect2);
